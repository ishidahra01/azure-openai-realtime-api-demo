...
This repo contains an example of how to implement GPT-4o Realtime API to your apps. 

- [GPT-4o Realtime API for Audio](#gpt-4o-realtime-api-for-audio)
	- [Features](#features)
...
The `RTClient` in the frontend receives the audio input, sends that to the Python backend which uses an `RTMiddleTier` object to interface with the Azure OpenAI real-time API.

This repository includes infrastructure as code and a `Dockerfile` to deploy the app to Azure Container Apps, but it can also be run locally as long as Azure OpenAI services are configured.

## Getting Started

You can [set it up locally](#local-environment).
...
2. Clone the repo (`git clone https://github.com/ishidahra01/azure-openai-realtime-api-demo`)
